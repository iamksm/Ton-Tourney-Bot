...
async def on_message(message):
    if 'https://discord.gg/' in message.content:
        ...
        await message.author.send(
            f"{message.author.mention} Don't send Invite links!")
    else:
...
async def server(ctx):
    ...
    if ctx.guild.premium_subscribers:
        ...
        del names
        embed.add_field(name="Current Server Boosters",
                        value='\n'.join(mentions),
                        inline=False)
    ...
    embed.add_field(
        ...
        value=local_datetime(
            ctx.guild.created_at).strftime("%A, %B %d %Y @ %H:%M:%S %p %Z"),
        inline=False)
...
async def whois(ctx, member: discord.Member):
    embed = discord.Embed(title=member.name,
                          ...
                          color=discord.Color.blue())
    embed.add_field(name="Name and Tag",
    ...
    embed.add_field(
        ...
        value=local_datetime(
            member.created_at).strftime("%A, %B %d %Y @ %H:%M:%S %p %Z"),
        inline=False)
    embed.add_field(
        name="Joined Server On",
        value=local_datetime(
            member.joined_at).strftime("%A, %B %d %Y @ %H:%M:%S %p %Z"),
        inline=False)
